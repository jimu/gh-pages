...
function ASSERT(assert, output) 
{
    if (enableAsserts) {
        ...
        //throw "STOP"
        //throw output || 'Assertion failed'
      //window.stop(); << THIS WILL PREVENT console.log and console.assert. AND breaks reloads
    }
}
 ...
 *  @param {String}  [font]
 *  @memberof Debug */
function debugText(text, pos, size=1, color='#fff', time=0, angle=0, font='monospace', align='center')
{
    ASSERT(typeof color == 'string', 'pass in css color strings'); 
    if (typeof color != 'string') throw 'pass in css color strings'; 
    debugPrimitives.push({text, pos, size, color, time:new Timer(time), angle, font, align});
}
...

    {
        ...
        debugPrimitives.forEach(p=>
        {
            ...
            if (p.text != undefined)
            {
                ...
                overlayContext.textAlign = p.align;
...
class FontImage
{
    ...
    drawTextMult(text, pos, mult=vec2(1), center)
    {
        this.drawTextScreenMult(text, worldToScreen(pos).floor(), mult.scale(cameraScale|1), center);
    }
     ...
     *  @param {Boolean} [center]
     */
    drawTextScreenMult(text, pos, mult, center)
    {
        const context = this.context;
        context.save();
        context.imageSmoothingEnabled = !canvasPixelated;
        ...
        const size = this.tileSize;
        const drawSize = size.add(this.paddingSize).multiply(mult);
        const cols = this.image.width / this.tileSize.x |0;
        (text+'').split('\n').forEach((line, i)=>
        {
            const centerOffset = center ? line.length * size.x * mult.x / 2 |0 : 0;
            for(let j=line.length; j--;)
            {
                // draw each character
                let charCode = line[j].charCodeAt(0);
                ...
                const drawPos = pos.add(vec2(j,i).multiply(drawSize));
                context.drawImage(this.image, x * size.x, y * size.y, size.x, size.y, 
                    drawPos.x - centerOffset, drawPos.y, size.x * mult.x, size.y * mult.y);
            }
        });
...
class TileLayer extends EngineObject
{
    ...
    */
    ...
    {
        debugText(`${this.name}: setData ${layerPos}, ${data}, ${redraw}`, vec2(-20,4), 0.7, '#fa0', 5);
        console.log(`${this.name}: setData ${layerPos}, ${data}, ${redraw}`, data);
...
//        debugText(`JAU1 ${this.name} - nothing written`, vec2(-20,3), 0.7, '#fa0');
    ...
    {
        console.log(`drawTileData(${layerPos}, ${clear})`);

        if (this.seq++ == 0)
        {
          this.data.forEach( (tileLayerData) => console.log(tileLayerData.color));
          console.log(`drawTileData(${layerPos}, ${clear}) data`, this.data);
        }
        ...
        console.log(layerPos, d.tile, d.color)
        if (d.tile != undefined)
        {
            ASSERT(mainContext == this.context, 'must call redrawStart() before drawing tiles');
            ...
            console.log(`${this.name}.drawTile(${layerPos}, ${clear}) DEFINED, ${d.tile}`, tileInfo);
        }
    }
...
function engineInit(gameInit, gameUpdate, gameUpdatePost, gameRender, gameRenderPost, imageSources=[], rootElement=document.body)
{
    ...
    function engineUpdate(frameTimeMS=0)
    {
        ...
        if (!headlessMode)
        {
            ...
            if (showWatermark)
            {
                ...
                watermarkY = 3;
                watermarkText(text);
                watermarkText(`cameraPos: ${cameraPos}`)
                watermarkText(`cameraScale: ${cameraScale}`)
                watermarkText(`tileSizeDefault: ${tileSizeDefault}`)
                watermarkText(`mainCanvasSize: ${mainCanvasSize}`)
                watermarkText(`canvasMaxSize: ${canvasMaxSize}`)
                watermarkText(`frame: ${frame}`)
                watermarkText(`time: ${time.toFixed(3)}`)
                watermarkText(`timeReal: ${timeReal.toFixed(3)}`)
                watermarkText(`paused: ${paused}`)
                watermarkText(`medals: ${Object.keys(medals).length}`)
    ...
    function watermarkText(text) {
      overlayContext.fillStyle = '#000';
      overlayContext.fillText(text, mainCanvas.width-3, watermarkY);
      overlayContext.fillStyle = '#fff';
      overlayContext.fillText(text, mainCanvas.width-3, watermarkY-1);
      watermarkY += 16;
    }
